...
std::string HelpMessage()
{
    ...
    strUsage += "  -conf=<file>           " + _("Specify configuration file (default: testcoin.conf)") + "\n";
    strUsage += "  -pid=<file>            " + _("Specify pid file (default: testcoind.pid)") + "\n";
    ...
    strUsage += "  -port=<port>           " + _("Listen for connections on <port> (default: 11710)") + "\n";
    ...
    strUsage += "  -rpcport=<port>        " + _("Listen for JSON-RPC connections on <port> (default: 11711)") + "\n";
...
strUsage += "\n" + _("Masternode options:") + "\n";
    ...
    strUsage += "  -anonymizetestcoinamount=<n> " + _("Keep N TestCoin anonymized (default: 0)") + "\n";
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. TestCoin is shutting down."));
    ...
    if (!lock.try_lock())
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. TestCoin is probably already running."), strDataDir));

    if (GetBoolArg("-shrinkdebugfile", !fDebug))
        ShrinkDebugFile();
    LogPrintf("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
    LogPrintf("TestCoin version %s (%s)\n", FormatFullVersion(), CLIENT_DATE);
    ...
    nAnonymizeTestCoinAmount = GetArg("-anonymizetestcoinamount", 0);
    if(nAnonymizeTestCoinAmount > 999999) nAnonymizeTestCoinAmount = 999999;
    if(nAnonymizeTestCoinAmount < 2) nAnonymizeTestCoinAmount = 2;
    ...
    LogPrintf("Anonymize TestCoin Amount %d\n", nAnonymizeTestCoinAmount);
