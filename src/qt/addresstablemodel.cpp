...
bool AddressTableModel::setData(const QModelIndex &index, const QVariant &value, int role)
{
    ...
    if(role == Qt::EditRole)
    {
        ...
        case Label:
            ...
            {
                wallet->SetAddressBookName(CTestCoinAddress(strTemp).Get(), value.toString().toStdString());
            }
            break;
        case Address:
            ...
            if(CTestCoinAddress(rec->address.toStdString()) == CTestCoinAddress(value.toString().toStdString()))
            {
                editStatus = NO_CHANGES;
                return false;
            }
            ...
            else if(wallet->mapAddressBook.count(CTestCoinAddress(value.toString().toStdString()).Get()))
            {
            ...
            {
                {
                    LOCK(wallet->cs_wallet);
                    // Remove old entry
                    wallet->DelAddressBookName(CTestCoinAddress(rec->address.toStdString()).Get());
                    // Add new entry with new address
                    wallet->SetAddressBookName(CTestCoinAddress(value.toString().toStdString()).Get(), rec->label.toStdString());
                }
            }
            break;
        }
    ...
    if(type == Send)
    {
        ...
        } else
        {
            ...
            // Check for duplicate addresses
            {
                ...
                wallet->SetAddressBookName(CTestCoinAddress(strAddress).Get(), strLabel);
            }
        }
    }
...
{
    {
        ...
        {
            CTestCoinAddress address_parsed(sAddr);
