...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nAnonymizeTestCoinAmount"))
        settings.setValue("nAnonymizeTestCoinAmount", 1000);
    nAnonymizeTestCoinAmount = settings.value("nAnonymizeTestCoinAmount").toLongLong();
    if (settings.contains("nDarksendRounds"))
        SoftSetArg("-darksendrounds", settings.value("nDarksendRounds").toString().toStdString());
    if (settings.contains("nAnonymizeTestCoinAmount"))
        SoftSetArg("-anonymizetestcoinamount", settings.value("nAnonymizeTestCoinAmount").toString().toStdString());
...
QVariant OptionsModel::data(const QModelIndex & index, int role) const
{
    if(role == Qt::EditRole)
    {
        ...
        case AnonymizeTestCoinAmount:
            return QVariant(nAnonymizeTestCoinAmount);
        case UseBlackTheme:
...
bool OptionsModel::setData(const QModelIndex & index, const QVariant & value, int role)
{
    ...
    if(role == Qt::EditRole)
    {
        ...
        case DarksendRounds:
            ...
            break;
        case AnonymizeTestCoinAmount:
            nAnonymizeTestCoinAmount = value.toInt();
            settings.setValue("nAnonymizeTestCoinAmount", nAnonymizeTestCoinAmount);
            emit AnonymizeTestCoinAmountChanged(nAnonymizeTestCoinAmount);
            break;
        default:
            break;
        }
