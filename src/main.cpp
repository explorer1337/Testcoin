...
unsigned int nStakeMinAge = 1 * 60; // 60sec
...
int64_t GetProofOfWorkReward(int nHeight, int64_t nFees)
{
    ...
    else if (nHeight > 2000 && nHeight <= 250000) {
...
int64_t GetProofOfStakeReward(const CBlockIndex* pindexPrev, int64_t nCoinAge, int64_t nFees)
{
    ...
    if (pindexBest->nHeight+1 > 1 && pindexBest->nHeight+1 <= 250000)  {
        nSubsidy = 25 * COIN;
    }
    else if (pindexBest->nHeight+1 > 250000 && pindexBest->nHeight+1 <= 400000)  {
        nSubsidy = 15 * COIN;
    }
    ...
    else if (pindexBest->nHeight+1 > 850000 && pindexBest->nHeight+1 <= 1750000) {
	// end game - further discussion needed
        nSubsidy = 3 * COIN;
    } else if (pindexBest->nHeight+1 > 1750000) {
	// end game - further discussion needed
        nSubsidy = 3 * COIN;
        nSubsidy >>= ((pindexBest->nHeight + 250000) / 1050000);
    }
...
// ----------- instantX transaction scanning -----------
    ...
    if (!fIsInitialDownload)
    {
        if(MasternodePayments)
        {
			...
			research = vtx[1].vout[i].scriptPubKey;
                        payee = vtx[1].vout[i].scriptPubKey;
                        CTxDestination address111;
                    ExtractDestination(research, address111);
                    CTestCoinAddress address222(address111);
		LogPrintf("Foreach : %s\n", address222.ToString().c_str());
			outval = vtx[1].vout[i].nValue;
                LogPrintf("Foreacham : %d\n", outval);
...
int64_t GetMasternodePayment(int nHeight, int64_t blockValue)
{
    ...
    if (nHeight < 3) {
	ret = 0;
    } else if (nHeight >= 3 && nHeight <= 250000) {
